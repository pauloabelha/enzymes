...
root_folder = '~/Desktop/3dwh_segmentation/';

folders = {'autosegmentation_core_1/', 'autosegmentation_core_2/', 'autosegmentation_core_3/', 'autosegmentation_core_4/'};
    ...
    for i=initial_pcl_ix:size(pcl_filenames,2)
        ...
        if ~isempty(findstr('out',pcl_filenames{i}))
            try
                ...
                WritePly(P,[root_folder folders{f} pcl_filenames{i}(1:end-3) 'ply']);
                disp(['Pointcloud ' root_folder folders{f} pcl_filenames{i} ' ' num2str(i) '/' num2str(size(pcl_filenames,2))]);
                tot_toc = tot_toc+toc;
                avg_toc = tot_toc/i;
