...
* 	-o: Followed by the input directory
...
void write_usage(void) {
	...
	write_comment_line("\t-o: Followed by the input directory"); 
...
void write_mesh_to_ply_file(std::string orig_filepath, Polyhedron mesh, Facet_with_id_pmap<std::size_t> segment_property_map, int n_segms, int k, float lambda) {
	...
	output << "element vertex " << mesh.size_of_vertices() << std::endl;
	...
	output << "element face " << mesh.size_of_facets() << std::endl;
...
int convert_meshes(std::string working_dir, std::string OUTPUT_DIR, std::vector<std::string> meshes_filenames, std::string ext_out) {
...
int convert_meshes_in_dir(std::string & INPUT_DIR, std::string OUTPUT_DIR, std::string INPUT_FILE_EXT) {	
	if (!INPUT_FILE_EXT.empty()) {
		...
		if (!convert_meshes(INPUT_DIR, OUTPUT_DIR, meshes_filenames_to_convert, "off")) {
...
int get_meshes_filenames(std::string INPUT_DIR, std::vector<std::string> & meshes_filenames) {
...
int main(int argc, char *argv[]){
	...
	if(!convert_meshes_in_dir(INPUT_DIR, OUTPUT_DIR, INPUT_FILE_EXT))
		return 0;
