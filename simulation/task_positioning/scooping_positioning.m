function [ elbow_pos, tool_relative_pos, tool_rot, action_tracker_pos ] = scooping_positioning( ptool, task_params, P )
    safe_dist = [0.01 0 0.025];
    %% get task params
    box_pos = task_params{1};
    box_size = task_params{2};
    box_thickness = task_params{3};
    tool_relative_pos = [0 0 0];
    tool_rot = [0 0 0];
    box_top_level = box_pos(3) + box_size(3) + box_thickness;
    ...
    zf = box_pos(3) + box_thickness + safe_dist(3) - min(P.v(:,3)) + max(P.v(:,1));
    elbow_pos = [xi 0 zf];
