function GenerateBoxSDF( root_folder, box_name, bottom_pose, width, length_box, height, thickness, description )
    ...
    system(['mkdir ' root_folder box_name]);    
    GenerateModelConfigFile([root_folder box_name], sdf_version, box_name, author_name, author_email, description);

    filepath = [root_folder box_name '/' box_name '.sdf'];
    ...
    size_0 = [length_box, width, thickness];
    GenerateLinkTag(fid, [box_name '_' num2str(0)], pos_0, size_0, 2000, 2000, 'WoodFloor');
    fprintf(fid,'\n');
    fprintf(fid,'\n');    
    pos_1 = [(-length_box/2)+thickness/2, 0, thickness + (height/2)];  
    ...
    size_3 = [length_box, thickness, height];
