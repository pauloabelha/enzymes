...
function [ptool,orig_grasp_centre,orig_vec_grasp,ptool_error] = ExtractPTool(SQ_grasp,SQ_action,mass,ERRORS_SQs)    
    %% check params
    if ~exist('ERRORS_SQs','var')
        ERRORS_SQs = [Inf Inf];
    end
    if ~exist('SQ_grasp','var') || ~exist('SQ_action','var')
        error('This function needs two SQs as its first two params');
    end
    ptool_error = sum(ERRORS_SQs);
    ...
    ptool_action = [SQ_action(1:5) SQ_action(9:10) SQ_action(11:12)];
